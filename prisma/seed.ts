      ...
      data: [
        {
          ...
          action: "初始化导入",
          actor: item.owner,
          content: `来自${item.sourceType}《${item.sourceTitle}》：${record.modelType}已创建，项目${record.industry}，区域${record.region}，等待解析入库。`,
        },
        {
          modelId: item.id,
          sourceType: item.sourceType,
          ...
          action: "工程问答分析",
          actor: "工程知识助手",
          content: `来自${item.sourceType}《${item.sourceTitle}》：已完成初步参数抽取，风险等级${item.riskLevel}，建议进一步进行来源复核。`,
        },
      ],
    });
